...
const findings = [
  ...
  ['F-03', 'Account privacy and library persistence', 'Implemented / migration pending', 'Profile access is server-guarded; cloud ownership comes from the authenticated user, local account state is cleared on logout/switch, and the versioned snapshot is protected by auth.uid() RLS.', 'pending'],
  ['F-04', 'PWA lifecycle', 'Implemented / install test pending', 'Production registration, update prompt, private-route cache bypasses, offline fallback, and required raster icons are present; deployed install/offline verification remains open.', 'pending'],
  ...
  ['F-08', 'API, route, and player hardening', 'Remediated', 'Strict bounded route validation, allowlisted player origins/paths, reduced iframe capabilities, consistent frame policy, bounded API behavior, stable public errors, and redacted logs are present.', 'verified'],
  ...
  ['F-10', 'Performance and observability', 'Implemented / production measurement pending', 'next/font, deferred player connection hints, image budgets, Web Vitals, privacy-safe client error events, and fresh local browser timing evidence are present.', 'pending'],
] as const
    ...
    <div className="mx-auto max-w-[1280px] px-5 py-12 sm:px-8 lg:px-12 lg:py-20">
      <header className="max-w-4xl">
        ...
        <p className="mt-6 max-w-2xl text-base leading-8 text-muted-foreground">
          This page records the status of the ten findings in the baseline audit. It separates code-level verification from actions that require a real Supabase project, deployed PWA checks, or production-device measurement.
        </p>
      </header>
      ...
      <section id="overview" className="mt-12 grid gap-4 sm:grid-cols-3" aria-label="Remediation summary">
        ...
        <SummaryCard icon={<ShieldCheck className="size-5 text-primary" />} label="Code remediations" value="8" copy="Configuration, reliability, security, accessibility, settings, PWA, and telemetry controls are implemented in code." />
        <SummaryCard icon={<Clock3 className="size-5 text-cyan" />} label="Deployment gates" value="3" copy="Apply/auth-test the library migration, verify deployed PWA behavior, and run production-device performance checks." />
      </section>
      ...
      <section id="verification" className="mt-16 grid gap-6 lg:grid-cols-[1.1fr_.9fr]">
        <div className="rounded-2xl border border-white/10 bg-surface p-6">
          ...
          <ul className="mt-5 space-y-3 text-sm leading-6 text-muted-foreground">
            <li><span className="font-semibold text-white">Passed:</span> `npm ci`, typecheck, lint, 24 Vitest files / 92 tests, production build, dependency audit, and 26 Playwright tests across Chromium and Mobile Chrome.</li>
            <li><span className="font-semibold text-white">Passed:</span> account boundary, strict backup import, route/player hardening, settings normalization, malformed episode handling, and the accessible mobile filter dialog.</li>
            <li><span className="font-semibold text-white">Measured locally:</span> production `/landing` browser timing in Chromium 1280×720 and Mobile Chrome 393×727; details are recorded in `PAGESPEED_BASELINE.md`.</li>
            <li><span className="font-semibold text-white">Not claimed:</span> live Supabase migration/account sync, deployed install/offline behavior, or a fresh production-device Lighthouse score.</li>
          </ul>
        </div>
        <div className="rounded-2xl border border-primary/20 bg-primary/[.06] p-6">
          ...
          <h2 className="mt-2 font-display text-2xl font-semibold text-white">Deployment evidence still required</h2>
          <p className="mt-3 text-sm leading-6 text-muted-foreground">Apply the versioned Supabase migration, run authenticated account-switch/logout sync tests, then verify deployed PWA behavior and real-device performance with valid provider credentials.</p>
