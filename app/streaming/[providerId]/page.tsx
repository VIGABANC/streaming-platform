...
export default async function ProviderPage({ params, searchParams }: { params: Promise<{ providerId: string }>; searchParams?: SearchParams }) {
  ...
  if (foundProvider) {
    const canonical = foundProvider.provider_name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')
    redirect(`/provider/${canonical}${query.type ? `?type=${encodeURIComponent(query.type)}` : ''}`)
  }
  const catalogProvider = foundProvider as WatchProvider | undefined
  if (!catalogProvider) {
  ...
  const label = catalogProvider.provider_name.replace('Amazon Prime Video', 'Prime Video').replace('HBO Max', 'Max')
      ...
      <div className="mx-auto max-w-[1440px]">
        ...
        <header className="mt-10 flex flex-col gap-7 border-b border-white/10 pb-10 md:flex-row md:items-end md:justify-between">
          <div className="flex items-center gap-5">
            <div className="grid size-16 place-items-center rounded-2xl bg-white p-3 shadow-2xl shadow-black/30"><Image src={providerLogo(catalogProvider.logo_path, 'w154')} alt="" width={64} height={64} className="max-h-11 w-auto object-contain" /></div>
